...
if ( !class_exists( "pluginbuddy_vidembed_admin" ) ) {
	class pluginbuddy_vidembed_admin {
		...
		function pluginbuddy_vidembed_admin(&$parent) {
			...
			if ( version_compare( $wp_version, '3.4.10', '>=') ) {
			add_action( 'media_buttons', array( &$this, 'add_post_button_3_5' ), 100);
			} else {
			add_action( 'media_buttons_context', array( &$this, 'add_post_button_pre_3_5' ) );
			}
		...
		function add_post_button_pre_3_5( $content ){
		...
		function add_post_button_3_5($editor_id = 'content'){
			$content = '<a href="post.php#TB_inline?inlineId=vidembed_form" class="thickbox" title="VidEmbed"><img src="' . $this->_pluginURL . '/images/vidembed-short.png" alt="VidEmbed" /></a>';
			echo $content;
		...
		function admin_menu() {
			if ( isset( $this->_parent->_series ) && ( $this->_parent->_series != '' ) ) {
				...
				add_menu_page($this->_parent->_name.' Getting Started', $this->_parent->_name, 'administrator', $this->_parent->_var, array(&$this, 'view_gettingstarted'), $this->_parent->_pluginURL.'/images/vidembed.png');
