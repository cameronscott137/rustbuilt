 ...
 * Plugin URI: http://ithemes.com/purchase/vidembed/
 * Description: Easily embed videos in posts,pages, and widgets.
 * Version: 1.0.30
 * Author: iThemes
 * Author URI: http://ithemes.com/
 * iThemes Package: vidembed
 *
 * Installation:
 * 
...
if (!class_exists("pluginbuddy_vidembed")) {
	class pluginbuddy_vidembed {
		...
		var $_url = 'http://ithemes.com/purchase/vidembed/';	// Purchase URL.
		...
		function ve_embed($ve_title,$ve_caption,$ve_url,$ve_type,$ve_width,$ve_height,$ve_auto) {
			...
			if ($ve_type == 'yt') {
				...
				$return .= '<iframe width="' . $ve_width . '" height="' . $ve_height . '" src="' . $yt_url . '" frameborder="0" allowfullscreen="true"></iframe>';
				...
				*/
				...
				$return .= '<iframe src="http://player.vimeo.com/video/' . $VEMid . '?title=' . $vimplayer['title'] . '&amp;byline=' . $vimplayer['byline']. '&amp;portrait=' . $vimplayer['portrait'] . '&amp;color=' . $vimplayer['color'] . '&amp;fullscreen=1&amp;autoplay=' . $ve_auto . '&amp;loop=0" width="' . $ve_width . '" height="' . $ve_height . '" frameborder="0" allowfullscreen="true"></iframe>';
...
function ithemes_vidembed_updater_register( $updater ) { 
    $updater->register( 'vidembed', __FILE__ );
}

add_action( 'ithemes_updater_register', 'ithemes_vidembed_updater_register' );
